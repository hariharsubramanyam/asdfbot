...
public class HDefaultState extends State{
	...
	public PriorityQueue<EncampmentSquare> sortencamps(MapLocation[] encamp, ArrayList<MapLocation> alliedEncamps){
		...
		for(MapLocation mL : encamp){
			...
			if(!isTaken(currEncamp, alliedEncamps) && mL.distanceSquaredTo(alliedHQ) > 3 && mL.distanceSquaredTo(enemyHQ) >= 2*mL.distanceSquaredTo(alliedHQ) && alliedEncamps.size() < 15){
...
//					this.sendCenterOfMassMessage(teamCM);
				...
				for(int d : directionOffsets){
					...
					Team teamOfMine = rc.senseMine(alliedHQ.add(lookingAtCurrently));
